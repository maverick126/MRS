...
*/
IF  (NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[sp_SalesEstimate_CopyDescriptionToNonstandardOption]') AND type in (N'P', N'PC')))
	BEGIN
		EXEC dbo.sp_executesql @statement = N'
		CREATE PROCEDURE [dbo].[sp_SalesEstimate_CopyDescriptionToNonstandardOption]
		AS
		BEGIN
			SET NOCOUNT ON;
		END';
	END
GO
ALTER PROCEDURE [dbo].[sp_SalesEstimate_CopyDescriptionToNonstandardOption]
...
BEGIN
	...
	DECLARE @brandid INT, @regionid INT, @Costprice DECIMAL(18,2), @sellprice DECIMAL(18,2), @productid VARCHAR(50), @storey INT
	...
	SELECT @NonStandardAreaName = AreaName, 
	       @NonStandardAreaid=areaid,
	       @NonStandardGroupName = GroupName,
	       @NonStandardGroupid=groupid,
	       @productareagroupid=ProductAreaGroupID 
   ...
   ELSE
    ...
    IF(@revisiondetailsid=0) -- add from option tree
       BEGIN
					...
					--SELECT	
								...
								--groupname		,
								--ProductID		,
								--ProductName+char(13)+'['+ProductID+']' AS 	ProductName,
								--ProductDescription,
					...
					--FROM		estimatedetails		ed
					--WHERE		estimatedetailsid=@tempestimatedetailsid					
				-- return the inserted item, this is for add to the list in the interface
	  END
  ELSE  -- add from existing product list, 
      BEGIN
                    SELECT @tempareaid=areaid, @tempgroupid=groupid
                    FROM   EstimateDetails
                    WHERE  EstimateDetailsID=@EstimateDetailsId
					-- get the description of old item
					INSERT INTO tbl_SalesEstimate_EstimateDetails
					...
					SELECT 
								...
								NULL,
								NULL,
								CASE WHEN ISNULL(fkid_NonStandardArea,0)=0 THEN @tempareaid ELSE fkid_NonStandardArea END,
								CASE WHEN  fkid_NonStandardGroup IS NULL THEN @tempgroupid ELSE fkid_NonStandardGroup END,
								--ISNULL(fkid_NonStandardArea, @tempareaid),
								--ISNULL(fkid_NonStandardGroup, @tempgroupid),
								additionalinfo,
								10, -- Default to NONE - NONE
								1,
								CostExcGST,
					...
					SELECT	
								...
								@NonStandardGroupName AS NonStandardGroupName,
								0 AS itemAccepted,
								ed2.fkid_NonStandardArea,
								ed2.fkid_NonStandardGroup,
								ed2.additionalinfo,
								ISNULL(pdc.PriceDisplayCodeID, 10) AS PriceDisplayCodeID,
								ISNULL(pdc.PriceDisplayCode + ' - ' + pdc.PriceDisplayDesc,'NONE - NONE') AS PriceDisplayDesc,	
								ed2.DerivedCost AS derivedcost,
...
-- keep the reference of copied items
      IF(@newID>0)
         BEGIN
           INSERT INTO tbl_SalesEstimate_SourceOfNSRCopy
           SELECT @revisionId,@EstimateDetailsId,@productareagroupid,@newID,@targetpagid,CASE WHEN @revisiondetailsid=0 THEN 'Available Options' ELSE 'Existing Selections' END ,GETDATE(),@userId
         END
