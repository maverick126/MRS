...
BEGIN
	...
	--Add all additional areas where customer has purchased but not in minimum area
	...
	WHERE fkid_SalesEstimate_EstimateHeader = @previousEstimateRevisionId AND areaid NOT IN (SELECT areaid FROM #tempHSA)
	      --AND EstimateDetails.StandardInclusion=1
   ...
   ,IsPromotionProduct		
	...
	#tempSI.groupid,
	#tempSI.groupname,
	#tempSI.productareagroupid,
	#tempSI.productname,
	#tempSI.promotionproduct
	FROM #tempSI
