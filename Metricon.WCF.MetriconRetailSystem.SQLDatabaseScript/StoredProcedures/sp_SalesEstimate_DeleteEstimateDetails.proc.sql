...
BEGIN
	...
	DECLARE @estimateDetailsId INT, @multiplepromotionid INT, @promotionid INT
	...
	SELECT @promotionid=e.PromotionID
	...
	IF(EXISTS(
				...
				INNER JOIN tblMultiplePromotion mp ON ed.ProductID=mp.BaseProductID AND fkidPromotionID=@promotionid
				WHERE EstimateDetailsID=@estimateDetailsId
		      )
	   )
	   BEGIN
				SELECT @multiplepromotionid=mp.idMultiplePromotion
				FROM EstimateDetails ed
				INNER JOIN tblMultiplePromotion mp ON ed.ProductID=mp.BaseProductID AND fkidPromotionID=@promotionid
				WHERE EstimateDetailsID=@estimateDetailsId
       END
	ELSE
	   SET @multiplepromotionid=0
	...
	BEGIN TRY
		...
		BEGIN TRANSACTION	
           ...
           ,[fkid_SalesEstimate_PredefinedDeletionReason])			
            ...
            -- remove the NSR reference from the same revison 
            
            DELETE FROM tbl_SalesEstimate_SourceOfNSRCopy
            WHERE fkid_salesestimate_estimatedetails=@revisionDetailsId
            
            
            -- if the item is master promo, should delete all promotion products as well
            IF(@multiplepromotionid>0)
               BEGIN
                  SELECT   
                          ed1.fkidEstimateDetails,
                          ed1.fkidStandardInclusions,
                          ed2.ProductAreaGroupID,
                          ed1.id_SalesEstimate_EstimateDetails
                  INTO    #tempremove
                  ...
                  INNER JOIN (SELECT * FROM PromotionProduct WHERE fkidMultiplePromotion=@multiplepromotionid) pp ON ed2.ProductAreaGroupID=pp.PagID
                  WHERE ed1.fkid_SalesEstimate_EstimateHeader=@revisionId 
                  
                  DELETE FROM  tbl_SalesEstimate_EstimateDetails
					WHERE id_SalesEstimate_EstimateDetails IN (Select id_SalesEstimate_EstimateDetails FROM #tempremove)
